...
import { MOCK_CONTACTS, getScoreBadge } from '@/lib/mock-data';
import { cn } from '@/lib/utils';

const today = new Date().toLocaleDateString('nl-BE', { weekday: 'long', day: 'numeric', month: 'long' });

const hotLeads = MOCK_CONTACTS.filter(c => c.status === 'hot').slice(0, 5);
const newToday = MOCK_CONTACTS.filter(c => c.createdAt === '2026-04-06').length;
const replies = MOCK_CONTACTS.filter(c => c.outreachStatus === 'replied').length;
const crossSignals = MOCK_CONTACTS.filter(c => c.signals.length >= 3).length;
    ...
    <div className="space-y-6">
              ...
              <div key={contact.id} className="flex items-center gap-4 px-5 py-3 hover:bg-secondary/30 transition-colors">
                ...
                <div className="flex gap-2 shrink-0">
                  {(['kunst', 'vermogen', 'luxe'] as const).map(type => {
                    ...
                    const colors = { kunst: 'bg-[#534AB7]/10 text-[#534AB7]', vermogen: 'bg-[#0fb57a]/10 text-[#0fb57a]', luxe: 'bg-[#f4a261]/10 text-[#f4a261]' };
                    return (
                      <div key={type} className={cn('w-6 h-6 rounded-md flex items-center justify-center text-[10px] font-mono font-medium', colors[type])}>
                        {count}
                      </div>
                    );
                  })}
                ...
                <div className="flex gap-2 shrink-0">
                  <button
                    onClick={() => setPushed(p => [...p, contact.id])}
                    disabled={isPushed}
                    className={cn(
                      'flex items-center gap-1 text-[10px] px-2.5 py-1.5 rounded-md border transition-colors',
                      isPushed
      ...
      <div className="bg-card border border-border rounded-xl p-5">
        ...
        <div className="space-y-2">
          {MOCK_CONTACTS.filter(c => c.signals.length >= 3).map(contact => (
            <div key={contact.id} className="flex items-center gap-3 py-2 px-3 bg-emerald-500/5 border border-emerald-500/10 rounded-lg">
              <div className="flex gap-1">
                {(['kunst', 'vermogen', 'luxe'] as const).map(type => {
                  const has = contact.signals.some(s => s.type === type);
                  const colors = { kunst: '#534AB7', vermogen: '#0fb57a', luxe: '#f4a261' };
                  return <div key={type} className={cn('w-2 h-2 rounded-full', has ? 'opacity-100' : 'opacity-20')} style={{ background: colors[type] }} />;
                })}
              </div>
